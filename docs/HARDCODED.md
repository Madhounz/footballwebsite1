...
| Season rollover on 1 July  | `src/lib/season.ts`                     | Never wrong, but it is a rule rather than a fact: the season is computed from the wall clock, not asked for                                                                |
| `tier`                     | `data/demo/competitions.json` → `tier`  | Only when a league is restructured. It exists so the Championship is not called a top flight; absent means 1                                                               |
...
at the cost that a provider error would repaint the table.

**Already done:** what the site says it covers used to be a sentence naming
five competitions, in four places. It is now built from `listCompetitions()`
every time it is printed — the `<meta>` description (`src/app/[locale]/layout.tsx`),
the about page, the navigation strip and the home rail — so it names a
competition the day its fixtures land and can never name one we do not hold.

## 2. Lookup tables — an edit is needed when the world changes

| Table                        | Where                                             | Size | What it does                                                                                                                                                                                     |
| ---------------------------- | ------------------------------------------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `COMPETITION_CODES`          | `src/lib/pipeline/normalize.ts`                   | 11   | Our id → football-data code and API-Football league id. A new competition starts here                                                                                                            |
| `CALENDAR_YEAR_COMPETITIONS` | `src/lib/season.ts`                               | 1    | Competitions whose season is one calendar year (the Brasileirão) rather than July→June. Wrong here means half a season fetched and a season label that reads 2026/27 for a season played in 2026 |
