...
- Never read `status === "live"` directly in a view. Use `isLive`/`isStaleLive` (`src/lib/live-status.ts`): a stored status is what a provider last said, and a match abandoned or postponed after kick-off leaves a record saying "live" that nothing corrects until the next full sync. Past `LIVE_LIMIT_MIN` the site stops claiming the match is in play and says `noUpdate` instead. It never rewrites the status — what happened is a fact, and facts come from providers.
...
- Match ingestion never creates teams from a bare name. Only the `--seed` step may create a team, from a provider's full team record, and it must log it. A match whose club we do not hold is skipped, which looks exactly like no match at all — so every path that ingests must report the names it could not resolve (`onUnknownTeam`), and the live refresh carries them out in `LiveRefreshOutcome.unknownTeams`. It matters most for the Champions League, whose field comes from leagues we do not cover.
...
- `NINETY_API_FOOTBALL_OFF=1` leaves API-Football out of the pipeline entirely, key and all. It is for a key that exists but should not be called — an account under review, most obviously, where a hundred refusals a day is a poor argument for being trusted again. It has to be set in **both** places the pipeline runs: the host's environment variables for `/api/sync`, and the repository variables for the Sync data workflow. Set in one only, the key is still being called from the other. Unset both to switch the provider back on.
