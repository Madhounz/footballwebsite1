...
- `NINETY_API_FOOTBALL_OFF=1` leaves API-Football out of the pipeline entirely, key and all. It is for a key that exists but should not be called — an account under review, most obviously, where a hundred refusals a day is a poor argument for being trusted again. Unset it to switch the provider back on.
